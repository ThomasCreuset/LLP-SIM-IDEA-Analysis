...
ROOT::VecOps::RVec<float> getEscapingTrackMass(
    ...
    float detectorHalfLength_m)
{
    ...
    for (size_t i = 0; i < pt.size(); ++i) {
        ...
        float L_barrel = std::numeric_limits<float>::infinity();

        if (abs_sin_theta > 0.0f) {
            L_barrel =
...
ROOT::VecOps::RVec<float> getPrimaryTrackBeta(
    const ROOT::VecOps::RVec<float>& tof_ps,
    const ROOT::VecOps::RVec<float>& theta,
    float detectorRadius_m,
    float detectorHalfLength_m)
{
    ...
    constexpr float c = 0.299792458f; // m/ns

    for (size_t i = 0; i < tof_ps.size(); ++i) {
        ...
        const float abs_sin_theta =
            std::abs(std::sin(theta[i]));

        const float abs_cos_theta =
            std::abs(std::cos(theta[i]));
        ...
        // Hypothetical path length to detector boundary
